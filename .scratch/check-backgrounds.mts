...
const compositions = (await db.collection(`books/${bookId}/compositions`).get()).docs
  .map((d) => d.data())
  .filter((c) => c.sourceId === book.activeSourceId && c.canonicalHash === book.canonical.hash);
let cost = 0;
const flagged: string[] = [];
for (const c of compositions) {
  const asset = (await db.doc(`books/${bookId}/visualAssets/layer__${c.compositionId}__background`).get()).data();
  const version = asset?.versions?.find((v: { versionId: string }) => v.versionId === asset.approvedVersionId);
  if (!version) {
