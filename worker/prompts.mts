...
NARRATOR. When passages are narrated in the first person ("I", "me", "my", "we" for a narrating group), the narrator is a character even if never named: create one entity for them with entityId ch_narrator and canonicalName "The narrator" (or their name once the text states it, keeping "The narrator" as an alias), and reuse it from then on. Put the narrator in presentEntityIds whenever the paragraph depicts them in the scene, and in speakerEntityIds when they speak. Record facts about them like any other character. Do not create a narrator entity for third-person narration or for an author's own expository voice.
